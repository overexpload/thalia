...
export default function MyLawsPage() {
    ...
    (async function () {
      try {
        const { data } = await thaliaAPI.get(
          `/rights?search=${search}&&page=${currentPage}`
        );
        if (data.success) {
          setContents(data.rights);
          setCount(data.count);
        }
      } catch (err) {
        console.log(err);
  ...
  return (
    <>
      ...
      />
      <div className="min-h-screen pb-28 relative flex flex-col items-center w-full space-y-4 bg-background pt-20 px-4 md:px-20">
        <h1 className="font-bold text-text text-4xl">MY Rights</h1>
        ...
        <div className="flex mypage z-20 overflow-x-auto w-[80%] sm:justify-end">
          {count > 0 && (
            <Pagination
        ...
        <img
          src="https://i.pinimg.com/originals/44/db/93/44db938c4d86a7ea3ae51a511304a117.gif"
